...
def SEResNet(input_shape=None,
    ...
    assert len(depth) == len(
        filters), "The length of filter increment list must match the length " \
                  "of the depth list."
...
def _resnet_block(input, filters, k=1, strides=(1, 1)):
    ...
    if strides != (1, 1) or init._keras_shape[channel_axis] != filters * k:
        init = Conv2D(filters * k, (1, 1), padding='same',
...
def _create_se_resnet(classes, img_input, include_top, initial_conv_filters,
    ...
    x = Conv2D(initial_conv_filters, (7, 7), padding='same', use_bias=False,
               strides=(2, 2),
               kernel_initializer='he_normal',
