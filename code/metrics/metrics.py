...
def SSDLoss(neg_pos_ratio=3, n_neg_min=0, alpha=1.0):
  '''
  ...
  def _SSDLoss(y_true, y_pred):
    ...
    aux_y_pred = tf.maximum(y_pred[:,:,:-12], 1e-15)
    # Compute the log loss
    log_loss = -tf.reduce_sum(y_true[:,:,:-12] * tf.log(aux_y_pred), axis=-1)
    classification_loss = tf.to_float(log_loss) # Output shape: (batch_size, n_boxes)

    absolute_loss = tf.abs(y_true[:,:,-12:-8] - y_pred[:,:,-12:-8])
    square_loss = 0.5 * (y_true[:,:,-12:-8] - y_pred[:,:,-12:-8])**2
    l1_loss = tf.where(tf.less(absolute_loss, 1.0), square_loss, absolute_loss - 0.5)
    smooth_l1_loss = tf.reduce_sum(l1_loss, axis=-1)
    localization_loss = tf.to_float(smooth_l1_loss) # Output shape: (batch_size, n_boxes)
    ...
    n_negative_keep = tf.minimum(tf.maximum(tf_neg_pos_ratio * tf.to_int32(n_positive), tf_n_neg_min), n_neg_losses)
    ...
    total_loss = (class_loss + tf_alpha * loc_loss) / tf.maximum(1.0, n_positive) # In case `n_positive == 0`
